...
//					gamePanel.getHeaderPanel().setPainterHeadPortrait(painter.getHeadPortrait());
					...
					for (int time = showTopicTime + drawTime + countDownTime; time > 0; time--) {
						...
						if (time > showTopicTime + drawTime) {				//倒计时阶段
						...
						} else if (time >= showTopicTime) {					//你画我猜阶段
							...
							if (time == showTopicTime + drawTime - showHintTime) {
								if (client == null) {
									ServerAction.sendData(DataType.HINT, topic[0].length() + "个字");
								} else {
									gamePanel.getHeaderPanel().setNotice(HeaderPanel.NoticeType.SHOW_HINT, topic[0].length() + "个字");
									ServerAction.sendDataExceptOne(DataType.HINT, topic[0].length() + "个字", client);
								}
							} else if (time == showTopicTime + drawTime - showHintTime - showHintTime) {
								if (client == null) {
									ServerAction.sendData(DataType.HINT, topic[0].length() + "个字，" + topic[1]);
								} else {
									gamePanel.getHeaderPanel().setNotice(HeaderPanel.NoticeType.SHOW_HINT, topic[0].length() + "个字，" + topic[1]);
									ServerAction.sendDataExceptOne(DataType.HINT, topic[0].length() + "个字，" + topic[1], client);
								}
							}
							if (checkGuessStatus()) {
								time = showTopicTime;
